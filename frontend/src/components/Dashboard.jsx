...
export default function Dashboard() {
  ...
  const fetchInsights = async () => {
    try {
      ...
      setTrendInsights(res.data.trend_insights || []);
    } catch (err) {
  ...
  return (
    <div className="w-full flex justify-center">
      <div className="w-full max-w-3xl space-y-8">
        ...
        <div className="bg-gray-800 text-white p-6 rounded-2xl shadow-lg">
          ...
          {goalData && (
            <div className="mt-4 p-4 bg-gray-700 rounded-lg">
              ...
              {goalData.ai && goalData.ai.used && (
                <p className="mt-2 text-xs text-gray-300">
                  Advice generated by {goalData.ai.provider || "an AI coach"}.
                </p>
              )}
              {goalData.ai && !goalData.ai.used && goalData.ai.error && (
                <p className="mt-2 text-xs text-amber-200">
                  AI unavailable ({goalData.ai.error}). Showing backup guidance.
                </p>
              )}
            </div>
          )}
        ...
        {/* Tips */}
        {(tips.length > 0 || trendInsights.length > 0) && (
          <div className="bg-gradient-to-br from-green-400 to-emerald-500 text-white shadow-lg rounded-2xl p-5 space-y-4">
            <div>
              ...
              {tips.length > 0 ? (
                ...
                <p className="text-sm text-emerald-100/80">
                  No general tips yet—track your spending for personalized nudges.
                </p>
              )}
            </div>
            {trendInsights.length > 0 && (
              <div className="border-t border-white/20 pt-4">
                <h4 className="text-md font-semibold mb-2">Trend Insights</h4>
                <ul className="space-y-2">
                  {trendInsights.map((trend, idx) => (
                    <li key={`${trend.category}-${idx}`} className="text-sm">
